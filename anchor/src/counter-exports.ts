...
import CounterIDL from '../target/idl/crud.json'
import type { Crud } from '../target/types/crud'

// Re-export the generated IDL and type
export { Crud, CounterIDL }
...
export function getCounterProgram(provider: AnchorProvider, address?: PublicKey): Program<Crud> {
  return new Program({ ...CounterIDL, address: address ? address.toBase58() : CounterIDL.address } as Crud, provider)
}
...
export function getCounterProgramId(cluster: Cluster) {
  switch (cluster) {
    ...
    case 'testnet':
      ...
      return new PublicKey('GWdouRWPPfWpamJSGM78ZKsGJteroFRGJpgESQqCwhhb')
    case 'mainnet-beta':
    default:
